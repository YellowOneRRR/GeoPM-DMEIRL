...
class GeolifeEnv:
    def __init__(self, bucket_name, data_dir, max_length):
    ...
    def get_traj(self):
        ...
        user_folders = [obj['Key'] for obj in objects['Contents'] if obj['Key'].endswith('/')]
        chosen_folder = random.choice(user_folders)
        traj_files = self.s3.list_objects_v2(Bucket=self.bucket_name, Prefix=chosen_folder + "Trajectory/")
        chosen_file = random.choice(traj_files['Contents'])['Key']

        obj = self.s3.get_object(Bucket=self.bucket_name, Key=chosen_file)
        traj = pd.read_csv(obj['Body'], sep=",", skiprows=6, header=None)
...
def lambda_handler(event, context):
    ...
    for grid in GRID:
        for delt in eplsion:
            i = 0
            env = GeolifeEnv(bucket_name='your-bucket-name', data_dir='Geolife_Trajectories_1.3/Data/', max_length=1000)
